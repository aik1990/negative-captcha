...
class NegativeCaptcha
  ...
  def initialize(opts)
    @secret = opts[:secret]||(RUBY_19 ? Digest::MD5.hexdigest("this_is_a_secret_key") : MD5.new("this_is_a_secret_key").hexdigest)
    ...
    @spinner = RUBY_19 ? Digest::MD5.hexdigest(spinner_text) : MD5.new(spinner_text).hexdigest
    ...
    @fields = opts[:fields].inject({}){ |hash, field_name|
      ...
      if RUBY_19
        ...
        MD5.new([field_name, @spinner, @secret]*'-').hexdigest
      end
